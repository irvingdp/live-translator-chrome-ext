...
- 原文與譯文字級分別調整
- 每行長度上下限、背景透明度、垂直位置皆可調，且在字幕進行中即時生效
- 字幕框寬度固定為「每行長度上限」，不隨句子長短伸縮
...
字幕框寬度由「每行長度上限」乘上一個字元寬換算而來，但仍以影片寬度的 92% 為上限。若把長度上限與字級同時調到很大，換算後的寬度會超過影片可用空間而被夾住，此時最長的一行仍可能折成兩行；調小其中一項即可。
