...
- The chunker relies on stabilized text being a prefix of the raw interim text.
- **Correction to an earlier claim in this plan.** Task 1 originally asserted that greedy left-to-right packing makes closed unit boundaries depend only on the prefix, so they can never move. That is overstated. Measured after implementation: no violations at widths 40, 60, 90, or 140, but 11 violations across widths 8-30, because `packSpans` decides piece N from piece N+1's final extent and `hardWrap`'s last-chunk word-break exemption lets N+1 shrink as a trailing partial word completes. Stability holds for the whole range `maxLineWidth` can be clamped to, which is why that clamp floor of 40 is load-bearing rather than cosmetic. Task 2 must not treat prefix stability as a general invariant: it keeps its own frozen text per closed unit and never re-reads a closed boundary from a later recomputation, which is what actually protects already-displayed text.
